...
const ListaClientes = ({
  ...
  return (
    <>
      {/* Mostrar lista solo si NO está el formulario abierto */}
      {!mostrarFormulario && (
        <div className={`bg-white rounded-lg shadow-md p-6 relative transition-all duration-300 ${mostrarDetalle && clienteDetalle ? 'pb-0 mb-0 min-h-[700px]' : ''}`}>
          {notification.message && (
            <div className={`fixed top-5 right-5 p-4 rounded-md shadow-lg text-white z-[60] ${notification.type === 'success' ? 'bg-green-500' : 'bg-red-500'}`}>{notification.message}</div>
          )}
          {/* ...existing code... */}
          {/* Aquí va TODO el contenido de la lista de clientes, igual que antes */}
          ...
          <div className="mb-4">
            <div className="relative">
              ...
              <input
                ...
                className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 shadow-sm"
                value={busqueda}
            ...
            <div className="flex items-center justify-between mt-4">
              ...
              <div className="flex space-x-2">
                <button
                  ...
                  className={`px-3 py-1 rounded-md shadow-sm ${paginaActual === 1 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                >
                  <FiChevronLeft />
                </button>
                {Array.from({ length: Math.min(5, totalPaginas) }, (_, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1 rounded-md shadow-sm ${paginaActual === pagina ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                    >
                      {pagina}
                    </button>
                  );
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-md shadow-sm ${paginaActual === totalPaginas ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                >
                  <FiChevronRight />
                </button>
              </div>
      ...
      {/* FormularioCliente solo cuando mostrarFormulario es true */}
      {mostrarFormulario && (
        <FormularioCliente
          ...
          onSave={async (datosFormulario) => {
            // Fuerza el campo tipo a estar presente y correcto, y prueba con mayúsculas si el backend lo requiere
            let tipoCliente = datosFormulario.tipo;
            if (!tipoCliente || (tipoCliente !== 'natural' && tipoCliente !== 'juridica')) {
              tipoCliente = clienteSeleccionado && clienteSeleccionado.tipo ? clienteSeleccionado.tipo : 'natural';
            }
            // Si el backend espera mayúsculas, descomenta la siguiente línea:
            // tipoCliente = tipoCliente.toUpperCase();
            const datosConTipo = { ...datosFormulario, tipoCliente: tipoCliente };
            // Debug: muestra el payload justo antes del fetch
            console.log('Payload enviado al backend:', datosConTipo);
      ...
      {mostrarDetalle && clienteDetalle && (
        <div className="absolute left-0 top-0 w-full h-full flex items-start justify-center z-50 bg-black bg-opacity-10">
