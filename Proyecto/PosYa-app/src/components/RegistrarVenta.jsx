...
const RegistrarVenta = ({ onCancel, onVentaRegistrada }) => {
  ...
  React.useEffect(() => {
    const fetchClientes = async () => {
      try {
        ...
        // Normalizar para que todos tengan cli_nombre, cli_apellido, cli_razon_social, cli_tipo_de_documento
        data = data.map(cli => ({
          ...cli,
          cli_nombre: cli.cli_nombre || '',
          cli_apellido: cli.cli_apellido || '',
          cli_razon_social: cli.cli_razon_social || '',
          cli_tipo_de_documento: cli.cli_tipo_de_documento || (cli.cli_razon_social ? 'NIT' : 'CC')
        }));
        setClientes(data);
      } catch (err) {
        setClientes([]);
  ...
  React.useEffect(() => {
    ...
    const cli = clientes.find(c => c.cli_identificacion === venta.identificacion_cliente);
  ...
  const handleProductoChange = (idx, e) => {
    ...
    let nuevos = productos.map((p, i) => {
      if (i !== idx) return p;
      if (name === 'nombre') {
        const prodSel = productosDisponibles.find(prod => prod.pro_nombre === value);
        if (prodSel) {
          return {
            ...p,
            nombre: value,
            precio_unitario: prodSel.pro_precio !== undefined ? Number(prodSel.pro_precio) : '',
            IVA_unitario: prodSel.pro_tasa_IVA !== undefined ? Number(prodSel.pro_tasa_IVA) : ''
          };
        } else {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      for (const prod of productos) {
        const prodInfo = productosDisponibles.find(p => p.pro_nombre === prod.nombre);
        if (!prodInfo) throw new Error(`Producto no encontrado: ${prod.nombre}`);
        const resp = await fetch(`http://localhost:3000/api/existencias/${prodInfo.pro_codigo}`);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div className="col-span-2">
            ...
            >
              ...
              {clientes.map(cli => {
                // Mostrar nombre completo si es natural, razón social si es jurídico, o solo identificación si no hay más datos
                let label = cli.cli_identificacion;
                if (cli.cli_razon_social) {
                  label = cli.cli_razon_social;
                } else if (cli.cli_nombre) {
                  label = `${cli.cli_nombre}${cli.cli_apellido ? ' ' + cli.cli_apellido : ''}`;
                }
                return (
                  <option key={cli.cli_identificacion} value={cli.cli_identificacion}>
                    {label}
                  </option>
                );
              })}
          ...
          <div>
            ...
            <input type="text" value={clienteSeleccionado ? (clienteSeleccionado.cli_tipo_de_documento || 'NIT') : ''} readOnly disabled className="border p-2 rounded w-full bg-gray-100 text-gray-700" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">NIT / Número de Identificación</label>
            <input type="text" value={clienteSeleccionado ? clienteSeleccionado.cli_identificacion : ''} readOnly disabled className="border p-2 rounded w-full bg-gray-100 text-gray-700" />
          </div>
        </div>
                      ...
                      >
                        ...
                        {productosDisponibles
                          .filter(p => {
                            ...
                            return (
                              prod.nombre === p.pro_nombre ||
                              !productos.some((prodSel, iSel) => prodSel.nombre === p.pro_nombre && iSel !== idx)
                            );
                          })
                          .map(p => (
