...
// Endpoint para obtener todos los clientes (naturales y jurídicos)
app.get('/api/clientes', (req, res) => {
  // Unir datos de CLIENTE con CLIENTE_NATURAL y CLIENTE_JURIDICO
  const query = `
    SELECT c.cli_identificacion,
           c.cli_direccion,
           c.cli_correo_electronico,
           c.cli_ciudad,
           c.cli_numero_telefonico,
           n.cli_tipo_de_documento,
           n.cli_nombre,
           n.cli_apellido,
           j.cli_razon_social
    FROM CLIENTE c
    LEFT JOIN CLIENTE_NATURAL n ON c.cli_identificacion = n.cli_identificacion
    LEFT JOIN CLIENTE_JURIDICO j ON c.cli_identificacion = j.cli_identificacion
    ORDER BY
      COALESCE(j.cli_razon_social, n.cli_nombre, c.cli_identificacion)
  `;
...
// Endpoint para obtener todos los productos (código, nombre, precio y tasa de IVA)
app.get('/api/productos', (req, res) => {
  db.all('SELECT pro_codigo, pro_nombre, pro_precio, pro_tasa_IVA FROM PRODUCTO ORDER BY pro_nombre', [], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(rows);
  });
});
